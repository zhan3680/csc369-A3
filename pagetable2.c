...
int allocate_frame(pgtbl_entry_t *p) {     
	...
	if ((victim_pte)->frame & PG_DIRTY){
            int new_offset = swap_pageout(frame, victim_pte->swap_off);      
            if(new_offset == INVALID_SWAP){
                exit(1);
            }
	    (victim_pte)->frame |= PG_ONSWAP;                    
	    (victim_pte)->swap_off = new_offset;
	    evict_dirty_count += 1;
	} else {
	    evict_clean_count += 1;
	}
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    pgtbl_entry_t *second_table = (pgtbl_entry_t *)(pgdir[idx].pde & ~PG_VALID);   
	
    // Use vaddr to get index into 2nd-level page table and initialize 'p'
    p = &second_table[PGTBL_INDEX(vaddr)];
	...
	hit_count += 1;
        ...
        // Frame on swap file.
	// Allocate frame, movepage in.
	int framenum = allocate_frame(p);
	p->frame = (framenum << PAGE_SHIFT) ;       
	if(swap_pagein(framenum, p->swap_off) != 0){
	    perror("SWAP PAGE FAULT");
	    exit(-1);  
	}                                           
	(p->frame) &= ~PG_DIRTY;                    
        (p->frame) &= ~PG_ONSWAP;
	...
	// First time access to the page, a cold-miss; want to call init_frame.
	int framenum = allocate_frame(p);           
